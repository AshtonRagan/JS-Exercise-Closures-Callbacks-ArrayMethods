...
function getFullNames(runners) {
  let diffarr = [];
  runners.forEach(ele => diffarr.push(`${ele.last_name}, ${ele.first_name}`));
...
function firstNamesAllCaps(runners) {
  let diffarr = runners.map(x => x.first_name.toUpperCase());
  //runners.forEach(ele => diffarr.push(ele.first_name.toUpperCase()));
  return diffarr;
}
...
function getRunnersByTShirtSize(runners, tsize) {
  let diffarr = runners.filter(ele => ele.shirt_size === tsize);
